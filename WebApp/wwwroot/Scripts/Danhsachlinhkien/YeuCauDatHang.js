$(".btnYeuCauDatHang").on("click", function () {
    var id = $(this).data("id");
    ...
    var ycdh = $(this).data("ycdh");
    if (ycdh > 0) {
        Swal.fire({
            icon: "warning",
            text: "Linh kiện hiện đang có " + ycdh + " yêu cầu đặt hàng",
            showDenyButton: true,
            confirmButtonText: "Tiếp tục yêu cầu đặt hàng",
            ...
            if (result.isConfirmed) {
                ...
                tableHtml += "<h4>XÁC NHẬN GỬI YÊU CẦU ĐẶT HÀNG</h4>";
                tableHtml += "</div>";
                tableHtml +=
                    '<div class="col-12 d-flex justify-content-center align-items-center"  style="margin-bottom: 30px">';
                tableHtml +=
                    '<a style="font-size:smaller">Người yêu cầu: <span style="font-size:medium" class="badge badge-warning">' +
                    hoten +
                    "</span></a>";
                tableHtml += "</div>";
                ...
                tableHtml +=
                    '<input class="form-control form-control-lg" type="number" id="soLuong" placeholder="Nhập số lượng cần đặt hàng..." style="margin-top: 10px;">';
                tableHtml +=
                ...
                $("#xacNhan").on("click", function () {
                    ...
                    if (soLuong.length < 1) {
                        ...
                        $.ajax({
                            url: "/Dathang/CreateYeuCauDatHang",
                            type: "POST",
                            data: {
                                linhkienid: id,
                                nguoithaotac: hoten,
                            ...
                            success: function (data) {
                                if (data.success) {
                                    Swal.fire(
                                        "Thành công!",
                                        "Bạn vừa gửi yêu cầu đặt hàng thành công",
                                        "success"
                                    ).then(function () {
                                    ...
                                    Swal.fire(
                                        "Không thành công!",
                                        "Số lượng yêu cầu không phù hợp",
                                        "error"
                                    );
                                }
                            },
